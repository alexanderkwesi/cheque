...
const Dashboard = () => {
  ...
  const [getidforuser, setidforuser] = useState('');
        ...
        const response = await axios.put(
          `${URL}/api/cheques/update-status/${chequeId}`,
          {
            ...
            failure_reason:
              ...
              "verification and compliance issue",
          },
          {
            headers: {
      ...
      switch (action) {
        case "download":
          ...
          const response = await axios.get(
            `${URL}/api/cheques/${chequeId}/download`, {
              user_id: getidforuser, chequeId: chequeId
            },
            {
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        case "delete":
          if (window.confirm("Are you sure you want to delete this cheque?")) {
            await axios.delete(`${URL}/api/cheques/${chequeId}`, {user_id : getidforuser, cheque_id: chequeId}, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
  ...
  // Enhanced Cheque Details Modal Component - Updated with smaller size and better focus
  const ChequeDetailsModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col transform transition-all duration-300 scale-100">
          {/* Modal Header */}
          <div className="flex justify-between items-center p-5 border-b border-gray-200 bg-white sticky top-0 z-10">
            <h2 className="text-xl font-bold text-gray-800">Cheque Details</h2>
            <button
              onClick={closeModal}
              className="text-gray-400 hover:text-gray-600 transition-colors p-2 hover:bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
              autoFocus
            >
              <svg
                className="w-5 h-5"
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {detailsLoading ? (
              <div className="text-center py-6">
                ...
                <p className="mt-3 text-gray-600">Loading cheque details...</p>
              </div>
            ) : detailsError ? (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                <p className="text-yellow-800 text-sm">{detailsError}</p>
              </div>
            ) : null}
          ...
          <div className="bg-gray-50 rounded-b-2xl p-4 border-t border-gray-200 sticky bottom-0">
            <h3 className="text-base font-semibold text-gray-800 mb-3">
              Actions
            </h3>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => handleChequeAction("download", displayData.id)}
                className="flex items-center space-x-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all duration-200 font-medium text-sm shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              {isCurrencyCompatible &&
                ...
                displayData.status !== "failed" && (
                  <button
                    ...
                    className="flex items-center space-x-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-all duration-200 font-medium text-sm shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
              ...
              <button
                onClick={() => handleChequeAction("resend", displayData.id)}
                className="flex items-center space-x-2 bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 transition-all duration-200 font-medium text-sm shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-opacity-50"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              <button
                onClick={() => handleChequeAction("delete", displayData.id)}
                className="flex items-center space-x-2 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-all duration-200 font-medium text-sm shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
