...
const Wallet = () => {
  ...
  const handleStripeDeposit = async () => {
    ...
    try {
      // Create checkout session with Stripe server
      const response = await axios.post(
  ...
  // Manual deposit fallback - creates a mock processed cheque
  const handleManualDeposit = async () => {
    ...
    try {
      // For manual deposits, we'll create a mock cheque entry in the backend
      // This ensures the balance calculation remains consistent
      const response = await axios.post(
  ...
  const handleWithdraw = async () => {
    ...
    try {
      // For withdrawals, we'll create a negative cheque entry
      const response = await axios.post(
        `${URL}/api/cheques/create`,
        {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      {/* Withdraw Modal remains the same */}
      {showWithdrawModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
